"use client";
import { useAccount, useWriteContract, useBlock, useWaitForTransactionReceipt } from "wagmi";
...
import { useERC20Token, useNFTs } from "@/app/Hooks/useAMM";
import { EUROPA_ROUTER, tokenSymbols, tokenAddresses, ROUTER_AQUADEX, MARKETPLACE_GOLD_NFT, MARKETPLACE_BRONZE_NFT, MARKETPLACE_SILVER_NFT } from "@/app/Utils/config"
...
const SwapAmm = () => {
    ...
    // NFT Balances 
    const { data: nft_gold_balance } = useNFTs(MARKETPLACE_GOLD_NFT, "balanceOf", [address]);
    const { data: nft_silver_balance } = useNFTs(MARKETPLACE_SILVER_NFT, "balanceOf", [address]);
    const { data: nft_bronze_balance } = useNFTs(MARKETPLACE_BRONZE_NFT, "balanceOf", [address]);
    ...
    useEffect(() => {
        if (address) {
            ...
            if (tokenA && tokenB) {
                ...
                console.log("look for nfts  ", nft_gold_balance);
                // new 
    ...
    const pathForSwap = (tokenA: string, tokenB: string) => {
        ...
        if (tokenA === 'DAI' || tokenA === 'USDP' || tokenA === 'USDC' || tokenA === 'USDT') {
            if (tokenB === 'DAI' || tokenB === 'USDP' || tokenB === 'USDC' || tokenB === 'USDT') {
                pathAmm = false;
            }
        }
        return pathAmm;
    ...
    const handleSwap = () => {
        const timeIs = (resultBlock?.data?.timestamp) ? (resultBlock?.data?.timestamp + BigInt(20000)) : BigInt(404);
        console.log("timestamp for swap ", timeIs);
        // Implement swapping logic here
        if (pathForSwap(tokenA, tokenB)) {
            // amm
            console.log(" DEX AMM Swap path: ", pathForPools(tokenAAddress, tokenBAddress));
            writeContract({
    ...
    const handleProvideLiquidity = () => {
        ...
        console.log("timestamp for add Liquidity ", tokenAAddress, tokenBAddress, address, parseEther(amountA), parseEther(amountB));
        ...
        <main>
            <div className={styles.container}>
                ...
                {toggleAMMFeatures.current === 'add' ? (
                    <div>  <div className={styles.input_container}>
                        ...
                        <div className={styles.button_container}>
                            ...
                            {tokenAllowance && BigInt(tokenAllowance) >= parseEther(amountA) ? (
                                <button className={styles.button_field} onClick={handleProvideLiquidity}>Add Liquidity</button>) : (
                                <button className={styles.button_field} onClick={handleLiquidityApprove}>Approve</button>)}
                ...
                {toggleAMMFeatures.current === 'nft' ? (
                    <div>  <div className={styles.input_container}>
                        <p>Gold NFT Holder</p>
                        <p className={styles.amount_balance}> {!nft_gold_balance ? ("0.0") : (typeof nft_gold_balance === 'bigint' && formatUnits(nft_gold_balance, 0))}</p>

                        <p>Silver NFT Holder</p>
                        <p className={styles.amount_balance}>  {!nft_silver_balance ? ("0.0") : (typeof nft_silver_balance === 'bigint' && formatUnits(nft_silver_balance, 0))}  </p>

                        <p>Bronze NFT Holder</p>
                        <p className={styles.amount_balance}>    {!nft_bronze_balance ? ("0.0") : (typeof nft_bronze_balance === 'bigint' && formatUnits(nft_bronze_balance, 0))} </p>
