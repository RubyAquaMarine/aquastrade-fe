/*
trading-app : not using but this is the core charts model to get me started 
*/

import { createChart } from "lightweight-charts";
import { FC, useEffect, useRef, useState, useLayoutEffect } from "react";
import {
  updateCandlesticSeries,
  updateVolumeSeries,
} from "../api/binanceUpdate";
...
import { wsController } from "../api/binance";
...
import { throttler } from "../Components/Form";
...
const ChartComponent: FC<{
  ...
  useLayoutEffect(() => {
    ...
    const handleResize = (apply: boolean) => {
      ...
      if (1536 < w) {
        apply && chart.applyOptions({ width: window.innerWidth - 700 });

        return window.innerWidth - 700;
      }

      if (w > 1280 && w <= 1536) {
        apply && chart.applyOptions({ width: window.innerWidth - 355 });
    ...
    if (data) {
      data.forEach((e) => {
        updateVolumeSeries(false, e, volumeSeries, colorSchemeVolumeSeries);
    ...
    const { removeMessageListener } = message((e) => {
      ...
      updateVolumeSeries(true, data, volumeSeries, colorSchemeVolumeSeries);
    });
  ...
  return (
    <>
      <div ref={container} />
      {size <= 1536 && authenticated && <MiniForm market={market} res={res} />}
...
const MiniForm: FC<{ market: BinanceStreams; res: string }> = ({
  ...
  const currentCoinAmount = typedUseSelector((store) => store.dbData.wallet)[
  ...
  const validateRequest = (c: (...args: any[]) => void) => {
    ...
    if (+amount.current!.value > +currentCoinAmount && side === "SELL") {
      return { exception: "No enough " + coin };
    }

    if (+price.current * +amount.current!.value > usdt && side === "BUY") {
